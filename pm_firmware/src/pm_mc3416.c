 /****************************************************************************************
pm_mc3416.c:   power management (PM) MC3416
...
enum status_code mc3416_set_mode(uint8_t mode)
{
	...
	status = pm_i2c_command_write_reg(mc3416_address, MC3416_REG_MODE, &b_mode, wr_length);
	
	if((b_mode & MC3416_MODE_WAKE) ==  MC3416_MODE_WAKE)
	{
		delay_ms(mc3416_wakeup_delay_ms);
		pm_usart_send_pc_message("wakeup delay finished!\r\n");
	}
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_set_mode: write_failed!\r\n");
	}
	return (status);
...
enum status_code mc3416_check_mode(void)
{
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr_length , MC3416_REG_DEVICE_STATUS, &b_mode, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_check_mode: read_failed!\r\n");
		return (status);
	}
	...
	{
		status = STATUS_BUSY;
		pm_usart_send_pc_message("mc3416_check_mode: STANDBY_MODE!\r\n");
	}
	return (status);
...
enum status_code mc3416_set_sampling_rate(void)
{
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr1_length, MC3416_REG_SAMPLE_RATE, &b_odr, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_set_sampling_rate: read_failed!\r\n");
		return (status);
	}
	b_odr = ((b_odr & MC3416_ODR_MASK) | MC3416_ODR);
	status = pm_i2c_command_write_reg(mc3416_address, MC3416_REG_SAMPLE_RATE, &b_odr, wr2_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_set_sampling_rate: write_failed!\r\n");
	}
...
enum status_code mc3416_set_range_resolution(void)
{
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr1_length, MC3416_REG_RANGE, &b_range, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_set_range_resolution: read_failed!\r\n");
		return (status);
	}
	
	b_range = ((b_range & MC3416_RANGE_MASK) | MC3416_RANGE_REG_VAL);
	status = pm_i2c_command_write_reg(mc3416_address, MC3416_REG_RANGE, &b_range, wr2_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_set_range_resolution: write_failed!\r\n");
	}
...
enum status_code mc3416_validate_chip(void)
{
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_CHIPID, &b_chipid, rd_length);
	
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_validate_chip: chipID_read_failed!\r\n");
		return (status);
	}
	
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_PRODUCT_CODE_L, &b_pcode, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_validate_chip: Pcode_read_failed!\r\n");
		return (status);
	}
	...
	{
		pm_usart_send_pc_message("mc3416_validate_chip: Chip_ID_incorrect!\r\n");
		return MC3416_ERROR;
	}

	if(b_pcode != MC3416_PCODE)
	{
		pm_usart_send_pc_message("mc3416_validate_chip: Product_Code_incorrect!\r\n");
		return MC3416_ERROR;
	}
...
enum status_code mc3416_read_axis(void)
{
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_XOUT_EX_L, &b_xl, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_sample_axis: axis_read_failed!\r\n");
	}
	status = pm_i2c_command_read_reg(mc3416_address, wr_length,MC3416_REG_XOUT_EX_H, &b_xh, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_sample_axis: axis_read_failed!\r\n");
	}
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_YOUT_EX_L, &b_yl, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_sample_axis: axis_read_failed!\r\n");
	}
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_YOUT_EX_H, &b_yh, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_sample_axis: axis_read_failed!\r\n");
	}
	...
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_ZOUT_EX_L, &b_zl, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_sample_axis: axis_read_failed!\r\n");
	}
	status = pm_i2c_command_read_reg(mc3416_address, wr_length, MC3416_REG_ZOUT_EX_H, &b_zh, rd_length);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("mc3416_sample_axis: axis_read_failed!\r\n");
	}
...
enum status_code pm_mc3416_init(void)
{
	...
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_init: no_chip_detected!\r\n");
		return (status);
	}
	status = mc3416_set_mode(MC3416_MODE_STANDBY);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_init: standby_mode_not_entered!\r\n");
		return (status);
	}
	status = mc3416_set_sampling_rate();
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_init: Sampling_rate_not_configured!\r\n");
		return (status);
	}
	status = mc3416_set_range_resolution();
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_init: Range_res_not_configured!\r\n");
		return (status);
	}
	...
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_init: Offset_reads_failed!\r\n");
		return (status);
	}
	status = mc3416_set_mode(MC3416_MODE_WAKE);
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_init: wake_mode_not_entered!\r\n");
		return (status);
	}
...
Returns status code indicating success or failure 
*****************************************************************************************/
enum status_code pm_mc3416_read_tilt(double *tilt_arg)
{
	...
	{
		pm_usart_send_pc_message("pm_mc3416_read_tilt: mode_check_failed!\r\n");
		return (status);
	}
	else if(status == STATUS_BUSY)
	{
		...
		if(status != STATUS_OK)
		{
			pm_usart_send_pc_message("pm_mc3416_read_tilt: Wakeup_failed!\r\n");
			return (status);
		}
	}
	...
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_read_tilt: Sample_axis_failed!\r\n");
		return (status);
	}
...
enum status_code pm_mc3416_standby(void)
{
	...
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_standby: failed to enter standby mode!\r\n");
	}
	return (status);
...
*****************************************************************************************/
void pm_mc3416_get_counts(int16_t *x_arg, int16_t *y_arg, int16_t *z_arg)
{
	*x_arg = xout;
	*y_arg = yout;
	*z_arg = zout; 
...
*****************************************************************************************/
void pm_mc3416_get_g_values( double *xg_arg, double *yg_arg, double *zg_arg)
{
	*xg_arg = xg;
	*yg_arg = yg;
	*zg_arg = zg; 
...
enum status_code pm_mc3416_calibrate(void)
{
	...
	if(status != STATUS_BUSY && status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_calibrate: mode_check_failed!\r\n");
		return (status);
	}
	else if(status == STATUS_BUSY)
	{
		...
		if(status != STATUS_OK)
		{
			pm_usart_send_pc_message("pm_mc3416_calibrate: Wakeup_failed!\r\n");
			return (status);
		}
	}
	status = mc3416_read_axis();
	if(status != STATUS_OK)
	{
		pm_usart_send_pc_message("pm_mc3416_calibrate: Read_axis_failed!\r\n");
		return (status);
	}
...
*****************************************************************************************/
void pm_mc3416_get_offsets(int16_t * xo_arg, int16_t *yo_arg, int16_t *zo_arg)
{
	*xo_arg = x_offset;
	*yo_arg = y_offset;
	*zo_arg = z_offset;
